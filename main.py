...
#obtain page links'  
def get_pages(main_url, number_of_pages, http_headers):
        response = requests.get(main_url, headers = http_headers)
        soup = BeautifulSoup(response.content, 'html.parser')
        pages = []
        for idx, link in enumerate(soup.select('div.pagination__links > a')):
                        link = link['href']
                        link = "https://www.propertyfinder.eg/" + link
                        pages.append(link)
                        if idx+1 == int(number_of_pages):
                                break
...
def save_data(data):
        ...
        df.to_csv('C:\\Users\\k_abo\\Desktop\\project\\properties.csv')
        return
...
if __name__ == '__main__':
        ...
        for page in get_pages(main_url, 10, http_headers):
                get_products('https://www.propertyfinder.eg/en/search?c=1&l=2254&ob=mr&page=4', http_headers)
        save_data(data)